<template>
  <div>
    <EducationSchoolsChart :credits="credits" :education="education" :labels="labels" />
    <p
      class="internationalPointsText"
    >* The international credits have other scales. These have been approximated to fit the Swedish credit system.</p>
    <div v-for="(school) in education" :key="school.name">
      <div class="container-fluid" v-if="school.active == true">
        <!-- Title: Name & Grade -->
        <h5
          class="schoolName inline"
          :style="{color: school.color}"
        >{{school.nameLong}} ({{school.nameShort}})</h5>
        <p
          class="lightgray inline marginLeftSmall"
        >| Avg. Grade: {{getAverageGrade[school.nameShort]}} of 5.0</p>
        ...
        <div class="container" v-if="school.exchangeFrom">
          <p class="inline" :style="{color: school.color}">{{school.exchangeFrom}}</p>
          <p class="lightgray inline marginLeftSmall">| {{school.exchangeYear}}</p>
        </div>
        ...
        <div v-for="degree in school.degrees" :key="degree.name">
          <div class="container">
            <p class="inline" :style="{color: school.color}">{{degree.name}}</p>
            <p class="lightgray inline marginLeftSmall">| {{degree.credits}} credits</p>
          </div>
        </div>
        ...
        <div v-for="(accomplishment, i) in school.accomplishments" :key="i">
          <div class="container">
            <p class="marginBottomSmall" :style="{color: school.color}">{{accomplishment.name}}</p>
      ...
      this.education.forEach((school, index) => {
        ...
        school.courses.forEach(course => {
          ...
          if (school.pointScale == "SE" || school.pointScale == "IT") {
            ...
            // Argentina have 21 credits per semester
