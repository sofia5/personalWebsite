const education = [{
        nameShort: "KTH",
        nameLong: "The Royal Institute of Technology",
        educationLevel: "University",
        schoolDescription: '"Since its founding in 1827, KTH Royal Institute of Technology in Stockholm has grown to become one of Europe’s leading technical and engineering universities, as well as a key centre of intellectual talent and innovation. We are Sweden’s largest technical research and learning institution and home to students, researchers and faculty from around the world dedicated to advancing knowledge."',
        schoolLink: "https://www.kth.se/en/om/fakta",
        active: false,
        pointScale: "SE",
        color: "#008FFB",
        degrees: [
            { name: "Bachelor of Science", credits: 180 },
            { name: "Master of Science", credits: 120 },
            {
                name: "Master of Science in Engineering",
                credits: 300
            }
        ],
        bachelorProgram: {
            name: "Information and Communication Technology",
            credits: 300
        },
        masterProgram: {
            name: "Human Computer Interaction and Design, EIT Digital",
            credits: 120
        },
        accomplishments: [{
            name: "EIT Digital scholarship",
            description: "Received the EIT Digital scholarship offered to the top applicants in each EIT Digital program and country, granting me 10.000 SEK / month."
        }],
        courses: [{
                name: "Mathematics, Basic Course ",
                credits: 4.5,
                courseReference: "SF1659",
                grade: "D",
                date: "2013-10-07",
                courseLevel: "Bachelor",
                area: "Math"
            },
            {
                name: "Digital Design",
                credits: 7.5,
                courseReference: "IE1204",
                grade: "C",
                date: "2013-10-28",
                courseLevel: "Bachelor",
                area: "Computer Science"
            },
            {
                name: "Japanese Studies and the Basic of Japanese",
                credits: 7.5,
                courseReference: "LS1380",
                grade: "B",
                date: "2013-12-14",
                courseLevel: "Bachelor",
                area: "Languages"
            },
            {
                name: "Calculus in One Variable",
                credits: 7.5,
                courseReference: "SF1625",
                grade: "D",
                date: "2014-01-11",
                courseLevel: "Bachelor",
                area: "Math"
            },
            {
                name: "Algebra and Geometry",
                credits: 7.5,
                courseReference: "SF1624",
                grade: "E",
                date: "2014-05-20",
                courseLevel: "Bachelor",
                area: "Math"
            },
            {
                name: "Discrete Mathematics",
                credits: 7.5,
                courseReference: "SF1610",
                grade: "C",
                date: "2014-05-27",
                courseLevel: "Bachelor",
                area: "Math"
            },
            {
                name: "Embedded Electronics",
                credits: 7.5,
                courseReference: "IE1206",
                grade: "C",
                date: "2014-06-05",
                courseLevel: "Bachelor",
                area: "Physics"
            },
            {
                name: "Algorithms and Data Structures",
                credits: 7.5,
                courseReference: "ID1020",
                grade: "B",
                date: "2014-10-31",
                courseLevel: "Bachelor",
                area: "Programming"
            },
            {
                name: "Electromagnetism and Waves",
                credits: 7.5,
                courseReference: "IF1613",
                grade: "D",
                date: "2014-10-31",
                courseLevel: "Bachelor",
                area: "Physics"
            },
            {
                name: "Information and Retrieval and Source Criticism",
                credits: 1.5,
                courseReference: "LI1012",
                grade: "P",
                date: "2014-12-10",
                courseLevel: "Bachelor",
                area: "Engineering / Academic skills"
            },
            {
                name: "Industrial Management, Basic Course",
                credits: 6.0,
                courseReference: "ME1003",
                grade: "B",
                date: "2015-01-15",
                courseLevel: "Bachelor",
                area: "Industrial Engineering & Leadership"
            },
            {
                name: "Computer Hardware Engineering",
                credits: 7.5,
                courseReference: "IS1200",
                grade: "D",
                date: "2015-03-16",
                courseLevel: "Bachelor",
                area: "Computer Science"
            },
            {
                name: "Calculus in Several Variable II",
                credits: 7.5,
                courseReference: "SF1670",
                grade: "E",
                date: "2015-03-16",
                courseLevel: "Bachelor",
                area: "Math"
            },
            {
                name: "Programming II",
                credits: 7.5,
                courseReference: "ID1019",
                grade: "E",
                date: "2015-03-20",
                courseLevel: "Bachelor",
                area: "Programming"
            },
            {
                name: "Networks and Communication",
                credits: 7.5,
                courseReference: "IK1203",
                grade: "B",
                date: "2015-03-20",
                courseLevel: "Bachelor",
                area: "Computer Science"
            },
            {
                name: "Team Leadership and Human Resource Management",
                credits: 6.0,
                courseReference: "ME2063",
                grade: "B",
                date: "2015-04-20",
                courseLevel: "Bachelor",
                area: "Industrial Engineering & Leadership"
            },
            {
                name: "Project IT",
                credits: 7.5,
                courseReference: "ID1003",
                grade: "P",
                date: "2015-06-09",
                courseLevel: "Bachelor",
                area: "Project / Thesis"
            },
            {
                name: "Modern Software Development",
                credits: 6.0,
                courseReference: "IV1303",
                grade: "P",
                date: "2015-06-09",
                courseLevel: "Bachelor",
                area: "Computer Science"
            },
            {
                name: "Programming I",
                credits: 7.5,
                courseReference: "ID1018",
                grade: "A",
                date: "2016-01-13",
                courseLevel: "Bachelor",
                area: "Programming"
            },
            {
                name: "Sustainable Development, ICT and Innovation",
                credits: 7.5,
                courseReference: "AG1815",
                grade: "A",
                date: "2016-04-15",
                courseLevel: "Bachelor",
                area: "Engineering / Academic skills"
            },
            {
                name: "Project Management: Leadership and Control",
                credits: 6.0,
                courseReference: "ME2015",
                grade: "B",
                date: "2016-06-23",
                courseLevel: "Bachelor",
                area: "Industrial Engineering & Leadership"
            },
            {
                name: "Degree Project in Information and Communication Technology, First Cycle",
                credits: 15.0,
                courseReference: "II123X",
                grade: "B",
                date: "2016-07-04",
                courseLevel: "Bachelor",
                area: "Project / Thesis"
            },
            {
                name: "Project Work II",
                credits: 15.0,
                courseReference: "SI2911",
                grade: "P",
                date: "2016-12-14",
                courseLevel: "Bachelor",
                area: "Project / Thesis"
            },
            {
                name: "Probability Theory and Statistics",
                credits: 6.0,
                courseReference: "SF1901",
                grade: "E",
                date: "2017-05-30",
                courseLevel: "Bachelor",
                area: "Math"
            },
            {
                name: "Human-Computer Interaction, Research Seminars",
                credits: 3.0,
                courseReference: "DH2632",
                grade: "B",
                date: "2018-06-27",
                courseLevel: "Master",
                area: "Human-Computer Science"
            },
            {
                name: "Interaction Programming and the Dynamic Web",
                credits: 7.5,
                courseReference: "DH2642",
                grade: "A",
                date: "2017-04-19",
                courseLevel: "Master",
                area: "Programming"
            },
            {
                name: "Multimodal Interaction and Interfaces",
                credits: 7.5,
                courseReference: "DT2140",
                grade: "B",
                date: "2017-01-20",
                courseLevel: "Master",
                area: "Programming"
            },
            {
                name: "Human-computer Interaction: Principles and Design",
                credits: 7.5,
                courseReference: "IC1007",
                grade: "C",
                date: "2016-12-21",
                courseLevel: "Master",
                area: "Human-Computer Science"
            },
            {
                name: "Methodology of Interaction Design",
                credits: 7.5,
                courseReference: "IC2005",
                grade: "B",
                date: "2017-01-16",
                courseLevel: "Master",
                area: "Human-Computer Science"
            },
            {
                name: "Research Methodology and Scientific Writing",
                credits: 7.5,
                courseReference: "II2202",
                grade: "A",
                date: "2017-02-06",
                courseLevel: "Master",
                area: "Engineering / Academic skills"
            },
            {
                name: "Technology-based Entrepreneurship",
                credits: 7.5,
                courseReference: "ME2062",
                grade: "A",
                date: "2017-06-27",
                courseLevel: "Master",
                area: "Innovation & Entrepreneurship"
            },
            {
                name: "Entrepreneurship for Engineers",
                credits: 6.0,
                courseReference: "ME2072",
                grade: "A",
                date: "2017-01-0",
                courseLevel: "Master",
                area: "Innovation & Entrepreneurship"
            },
            {
                name: "Business Development Lab of Entrepreneurship Engineers",
                credits: 9.0,
                courseReference: "ME2073",
                grade: "A",
                date: "2017-06-02",
                courseLevel: "Master",
                area: "Innovation & Entrepreneurship"
            },
            {
                name: "Summer Course- Entrepreneurship for Engineers",
                credits: 4.0,
                courseReference: "ME2078",
                grade: "B",
                date: "2017-07-02",
                courseLevel: "Master",
                area: "Innovation & Entrepreneurship"
            }
        ]
    },
    {
        nameShort: "SU",
        nameLong: "Stockholm University",
        educationLevel: "University",
        schoolDescription: '"In a changing and globalised world, Stockholm University contributes to the development of a sustainable democratic society through knowledge, enlightenment and the pursuit of truth, based on critical thinking. Ranked among the world’s top 100 universities, Stockholm University is one of Europe\'s leading centres for higher education and research in human science and science."',
        schoolLink: "https://www.su.se/english/about",
        active: false,
        pointScale: "SE",
        color: "#00E396",
        bachelorProgram: {
            name: "Business Administration",
            credits: 180
        },
        courses: [{
                name: "Accounting I",
                credits: 7.5,
                courseReference: "FE1403",
                grade: "E",
                date: "2018-11-02",
                courseLevel: "Bachelor",
                area: "Economics"
            },
            {
                name: "Marketing I",
                credits: 7.5,
                courseReference: "FE1303",
                grade: "C",
                date: "2018-12-05",
                courseLevel: "Bachelor",
                area: "Marketing"
            }
        ]
    },
    {
        nameShort: "ITBA",
        nameLong: "Instituto Tecnólogico de Buenos Aires",
        educationLevel: "University",
        schoolDescription: '"Instituto Tecnológico de Buenos Aires (ITBA) was founded on November 20th, 1959 and, on account of Law 14.557, it became one of the first private universities of Argentina."',
        schoolLink: "https://www.itba.edu.ar/institutional/?lang=en",
        active: false,
        pointScale: "AR",
        color: "#FEB019",
        exchangeFrom: "KTH",
        exchangeYear: 2015,
        coursesComment: "All courses taken in Spanish",
        courses: [{
                name: "Occupational and environmental studies",
                credits: 3,
                courseReference: "12.83",
                grade: 7.75,
                date: "2015-07-25",
                courseLevel: "Bachelor",
                area: "Engineering / Acedemic skills"
            },
            {
                name: "Operative systems",
                credits: 6,
                courseReference: "72.11",
                grade: 5.25,
                date: "2015-07-25",
                courseLevel: "Bachelor",
                area: "Computer Science"
            },
            {
                name: "Human-Computer Interactions (HCI)",
                credits: 6,
                courseReference: "72.36",
                grade: 6.75,
                date: "2015-07-25",
                courseLevel: "Bachelor",
                area: "Human-Computer Science"
            },
            {
                name: "Commercialization",
                credits: 3,
                courseReference: "81.20",
                grade: 8.75,
                date: "2015-07-25",
                courseLevel: "Bachelor",
                area: "Marketing"
            },
            {
                name: "Regular Spanish B1",
                credits: 3,
                courseReference: "99.51",
                grade: 8.0,
                date: "2015-07-25",
                courseLevel: "Bachelor",
                area: "Languages"
            }
        ]
    },
    {
        nameShort: "VU",
        nameLong: "Villanova University",
        educationLevel: "University",
        schoolDescription: '"Villanova University is a Roman Catholic institution of higher learning founded by the Order of Saint Augustine in 1842. Within this suburban campus close to Philadelphia, Villanova provides a comprehensive education rooted in the liberal arts; a shared commitment to the Augustinian ideals of truth, unity and love; and a community dedicated to service to others."',
        schoolLink: "https://www1.villanova.edu/main/about.html",
        active: false,
        pointScale: "US",
        color: "#FF4560",
        accomplishments: [{
                name: "Dr. Peter Wallenberg's scholarship",
                description: "Received The Dr. Peter Wallenberg scholarship as one of four students in Sweden 2012. The scholarship funded the Villanova studies."
            },
            {
                name: "Dean's list, both semesters",
                description: "Was among the students recognized for academic achievement during a term by the Dean of Villanova."
            }
        ],
        courses: [{
                name: "Ancients",
                credits: 3,
                courseReference: "ACS1000",
                grade: "A-",
                date: "",
                courseLevel: "Bachelor",
                area: "Religion"
            },
            {
                name: "Calculus I",
                credits: 4,
                courseReference: "MAT1500",
                grade: "A-",
                date: "",
                courseLevel: "Bachelor",
                area: "Math"
            },
            {
                name: "Knowledge, Reality, Self",
                credits: 3,
                courseReference: "PHI1000",
                grade: "A-",
                date: "",
                courseLevel: "Bachelor",
                area: "Philosophy"
            },
            {
                name: "International Relations",
                credits: 3,
                courseReference: "PSC1200",
                grade: "A-",
                date: "",
                courseLevel: "Bachelor",
                area: "Politics"
            },
            {
                name: "Intermediate Spanish I",
                credits: 3,
                courseReference: "SPA1121",
                grade: "A",
                date: "",
                courseLevel: "Bachelor",
                area: "Languages"
            },
            {
                name: "Moderns",
                credits: 3,
                courseReference: "ACS1001",
                grade: "A",
                date: "",
                courseLevel: "Bachelor",
                area: "Religion"
            },
            {
                name: "Survey of Communication Studies",
                credits: 3,
                courseReference: "COM1000",
                grade: "A-",
                date: "",
                courseLevel: "Bachelor",
                area: "Communication"
            },
            {
                name: "Computing with Images",
                credits: 3,
                courseReference: "CSC1040",
                grade: "A-",
                date: "",
                courseLevel: "Bachelor",
                area: "Photo Editing"
            },
            {
                name: "Intro to Tactical Leadership",
                credits: 3,
                courseReference: "MS0102",
                grade: "A-",
                date: "",
                courseLevel: "Bachelor",
                area: "Military Science"
            },
            {
                name: "General Psychology",
                credits: 3,
                courseReference: "PSY1000",
                grade: "B+",
                date: "",
                courseLevel: "Bachelor",
                area: "Psychology"
            },
            {
                name: "Intermediate Spanish II",
                credits: 3,
                courseReference: "SPA1122",
                grade: "A-",
                date: "",
                courseLevel: "Bachelor",
                area: "Languages"
            }
        ]
    },
    {
        nameShort: "UNITN",
        nameLong: "University of Trento",
        educationLevel: "University",
        schoolDescription: "The University of Trento was founded in 1962 and has always aimed at building alliance and reciprocal efficiency with Italian and foreign institutions and organizations. In 1982, the University (until then private) became public, with a statute that guaranteed self-government. ",
        schoolLink: "https://www.unitn.it/en/ateneo/151/university",
        active: false,
        pointScale: "IT",
        color: "#775DD0",
        exchangeFrom: "KTH",
        exchangeYear: 2017,
        accomplishments: [{
            name: "Cum Laude scholar",
            description: "Graduated from my master's degree with high grades, receiving the title Cum Laude 'with great praise' on the diploma."
        }],
        courses: [{
                name: "Affective Computing",
                credits: 6,
                courseReference: "155004",
                grade: 31,
                date: "2018-01-09",
                courseLevel: "Master",
                area: "Human-Computer Science"
            },
            {
                name: "E-learning",
                credits: 6,
                courseReference: "155009",
                grade: 28,
                date: "2018-01-10",
                courseLevel: "Master",
                area: "Human-Computer Science"
            },
            {
                name: "Design Epistemology and Ethics",
                credits: 6,
                courseReference: "155011",
                grade: 30,
                date: "2018-01-16",
                courseLevel: "Master",
                area: "Human-Computer Science"
            },
            {
                name: "Innovation and Enterpreneurship Studies in ICT",
                credits: 6,
                courseReference: "145623",
                grade: 28,
                date: "2018-01-26",
                courseLevel: "Master",
                area: "Innovation & Entrepreneurship"
            },
            {
                name: "Prototyping Interactive Systems",
                credits: 6,
                courseReference: "155002",
                grade: 30,
                date: "2018-02-13",
                courseLevel: "Master",
                area: "Human-Computer Science"
            },
            {
                name: "Final thesis",
                credits: 30,
                courseReference: "145623",
                grade: 30,
                date: "2018-07-25",
                courseLevel: "Master",
                area: "Project / Thesis"
            }
        ]
    }
];

export default education;